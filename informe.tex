...
\begin{abstract}
Este documento resume los 12 experimentos realizados en el proyecto FCC-DNLS, un modelo de espacio-tiempo discreto en una red FCC tridimensional con cero par\'ametros libres. El modelo predice que un colapso espectral irreversible genera tiempo emergente, solitones estables, un umbral de auto-atrapamiento (Norma=4), y una conexi\'on natural con la funci\'on zeta de Epstein y la distribuci\'on de n\'umeros primos. Se incluyen resultados de validaci\'on observacional con datos JWST y SPARC, as\'i como 4 preguntas abiertas y pasos a seguir.
\end{abstract}
...
\begin{itemize}
  ...
  \hline L\'inea cr\'itica del modelo FCC-DNLS: $\text{Re}(s) = 3/4$ (dimensi\'on efectiva 3/2)
\end{itemize}

\section{Experimento C1b: Error scaling de la funci\'on theta}

\subsection{Descripci\'on}
Convergencia del error en la identidad $\theta(t) = t^{-3/2}\theta(1/t)$ en funci\'on de $N_{\max}$ y $t$.

\subsection{Resultados}
\begin{center}
\begin{tabular}{ccccc}
\toprule
$N_{\max}$ & $N_{\text{sitios}}$ & Error & $N_{\max}^{-1}$ & $N_{\max}^{-2}$ \\
\midrule
2 & 33 & $2.12\times10^{-10}$ & 0.500000 & 0.250000 \\
3 & 75 & $2.12\times10^{-10}$ & 0.333333 & 0.111111 \\
4 & 141 & $2.12\times10^{-10}$ & 0.250000 & 0.062500 \\
5 & 231 & $2.12\times10^{-10}$ & 0.200000 & 0.040000 \\
6 & 345 & $2.12\times10^{-10}$ & 0.166667 & 0.027778 \\
\bottomrule
\end{tabular}
\end{center}

\begin{itemize}
  \item Convergencia exponencial (dominada por $\exp(-\pi n^2 t/2)$)
  \item No hay convergencia tipo ley de potencia
  \item Error dominado por precisi\'on num\'erica desde $N_{\max}=2$
\end{itemize}
...
\section{Experimento C3: Multipletes de Weyl D3 y 33 canales}

\subsection{Descripci\'on}
Construcci\'on de las \'orbitas del grupo de Weyl D3 (orden 24) y an\'alisis de las degeneraciones del Laplaciano FCC.

\subsection{Resultados}
\begin{itemize}
  \item Grupo de Weyl D3: orden 24 (permutaciones S3 + signos con producto = 1)
  \item Grupo de simetr\'ia completo: Oh (orden 48)
  \item Orbitas de Weyl: $(0,0,0)\to1$, $(1,1,0)\to12$, $(2,0,0)\to6$, $(2,1,1)\to24$, $(2,2,0)\to12$
  \item Primeros 33 autovalores \'unicos del Laplaciano FCC (N=8, 256 sitios) corresponden a multipletes D3
  \item Espectro determinado por teor\'ia de representaciones del grupo de Weyl D3
\end{itemize}

\section{Experimento C4: Estad\'istica GUE en primos y espectro FCC}

\subsection{Descripci\'on}
An\'alisis de espaciamientos del Laplaciano FCC (N=8), huecos entre primos hasta 2 millones ($N=148,933$ primos), comparaci\'on con GUE te\'orico y matriz GUE aleatoria.

\subsection{Resultados}
\begin{center}
\begin{tabular}{ccccc}
\toprule
$s$ & $P_{\text{FCC}}(s)$ & $P_{\text{primos}}(s)$ & $P_{\text{GUE}}(s)$ & $P_{\text{Poisson}}(s)$ \\
\midrule
0.07 & 0.0118 & 0.0000 & 0.0276 & 0.9335 \\
0.35 & 0.1036 & 0.0559 & 0.1313 & 0.7047 \\
0.70 & 0.2515 & 0.1640 & 0.2393 & 0.4966 \\
1.05 & 0.1896 & 0.1971 & 0.3093 & 0.3500 \\
1.40 & 0.1657 & 0.2086 & 0.3354 & 0.2467 \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Estad\'istica KS}
\begin{itemize}
  \item GUE matriz (200$\times$200): $D = 0.047$ (control)
  \item Primos: $D = 0.16$ vs GUE
  \item Poisson: $D = 0.213$ vs GUE (control)
  \item FCC Laplaciano (N=8): $D = 0.25$ (solo 12 valores \'unicos, N/S)
  \item Repulsi\'on a $s\to 0$ confirmada en primos
  \item La conexi\'on GUE requiere DNLS (no-lineal), no Laplaciano lineal (integrable $\to$ Poisson)
\end{itemize}

\section{Experimento C4b: Test GUE masivo en primos (1.27M)}

\subsection{Descripci\'on}
Extensi\'on del an\'alisis C4 con 1,270,607 primos (criba hasta 20M) y matriz GUE 400$\times$400 como control, para determinar si KS converge a 0 con m\'as primos.

\subsection{Resultados}
\begin{center}
\begin{tabular}{lccc}
\toprule
Estad\'istica & Primos & GUE 400$\times$400 & Poisson \\
\midrule
$N$ & 1,270,607 & 399 & 50,000 \\
KS vs GUE & 0.1655 & 0.0627 & 0.2137 \\
$P(s<0.05)$ & 0.0000 & 0.0020 & 0.0488 \\
$\langle s\rangle$ & 1.0000 & 1.0000 & 1.0001 \\
$\sigma(s)$ & 0.8334 & -- & -- \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item KS estable desde $N=500$ hasta $N=1,270,607$: $\mathbf{0.1655}$ (no mejora)
  \item Repulsi\'on a $s\to 0$ confirmada: $P(s<0.05)=0$ (id\'entico a GUE)
  \item Pero la distribuci\'on NO es GUE exacta: la cola es m\'as pesada
  \item \textbf{Conclusi\'on:} la repulsi\'on es genuina, pero la estad\'istica exacta requiere el DNLS no-lineal, no el Laplaciano lineal FCC (que es integrable $\to$ Poisson)
\end{itemize}
...
Ajuste con \texttt{curve\_fit} para las 175 galaxias SPARC:
\begin{center}
\begin{tabular}{lrr}
\toprule
...
EXP: $Y=0.46$, $V_{\text{asy}}=134$ km/s, $r_c=6.83$ kpc, $\chi^2=1.25$.\\
LAP: $\chi^2=4.54$ (antiguo modelo). NFW: $\chi^2=1.39$.
...
\begin{itemize}
  ...
  \item Si completa $\to$ el $z$ espectral desaparece $\to$ JWST sin explicaci\'on
  \item Si asint\'otico $\to$ universo siempre en transitorio $\to$ JWST natural, pero CMB problem\'atica
\end{itemize}
...
\begin{itemize}
  ...
  \item Umbral Norma=4: auto-atrapamiento \checkmark
...
\subsection{Extensiones --- RESULTADOS MIXTOS}
\begin{itemize}
  ...
  \item \textbf{C (Primos/Zeta):} Ecuaci\'on funcional verificada exactamente. L\'inea cr\'itica $\text{Re}(s)=3/4$. Repulsi\'on GUE confirmada pero no exacta (KS=0.16).
\end{itemize}

\subsection{Tensi\'on observacional (RESUELTA)}
\begin{itemize}
  ...
  \item \textbf{C4b GUE masivo:} 1.27M primos confirman repulsi\'on pero KS=0.1655 (no converge a 0). La conexi\'on GUE requiere DNLS.
...
\begin{enumerate}
  ...
  \item \textbf{Conexi\'on Laplaciano lineal (Poisson) $\to$ DNLS (GUE)?} La estad\'istica del Laplaciano FCC es Poisson. La conexi\'on GUE requiere la no-linealidad del DNLS. C4b confirma que primos tienen repulsi\'on pero no son GUE exacto.
...
\begin{enumerate}
  \item \textbf{Verificar GUE en DNLS:} Diagonalizar operador completo $L$ + no-linealidad $\gamma|\psi|^2$, medir espaciamientos. Pendiente.
  \item \textbf{Normalizaci\'on temporal:} Medir $\tau$ en N=24+ (miles de sitios) para escalamiento cosmológico. Pendiente.
  \item \textbf{Publicaci\'on:} SPARC resuelto. E137 candidato viable. Preprint con resultados A, B, C, SPARC, E137.
\end{enumerate}
